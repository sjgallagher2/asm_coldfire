...
// Most operands will fit into this format. Others must be handled manually.


int decode_coldfire(const uint8_t *buffer, int len, int pc, uint8_t *asm_buf);
...
const uint8_t buffer[20] = {0x66,0xec,0xb6,0x82,0x66,0x62,0xb8,0x82,0x66,0x5e,0x72,0x18,0xe3,0xaa,0xcc,0xbc,0x00,0xff,0xff,0xff};
const uint8_t buffer2[58] = {0x40,0xe7,0x46,0xfc,0xde,0xad,
                             0x4e,0x71,
                             0x4e,0x72,
                             0x4e,0x75,
                             0x4e,0x73,
                             0x4a,0xfc,
                             0x00,0x80,0xbe,0xef,0xde,0xee,
                             0x00,0xc1,
                             0x02,0xc1,
                             0x0a,0x80,0xbe,0xef,0xde,0xee,
                             0x02,0x80,0xbe,0xef,0xde,0xee,
                             0x0c,0x00,0xbe,0xef,0xde,0xee,
                             0x04,0x80,0xbe,0xef,0xde,0xee,
                             0x23,0x00,
                             0x10,0xaf,0x50,0x20,
                             0x20,0x20};

#define _BUFFER_SIZE 256
const uint8_t buffer3[_BUFFER_SIZE] = {
  
};

#define ASM_BUF_LEN     80

/* 
 * TODO
 *  Floating point instructions are missing a lot
 *  Check implementation of effective addresses when instruction is longword
 *  Finish effective addressing modes
 *  Add effective addressing incomplete check
 */

int main()
{
    printf("\tM68K ColdFire Decoder\n");
    const int len = 20; // Buffer length in bytes
    int pc = 0;

    uint8_t asm_buf[ASM_BUF_LEN] = {0};
    while(pc+1 < len){
        uint8_t instr_len = decode_coldfire(buffer,len,pc,asm_buf);
        if(instr_len <= 0){
            break;
        }
        
        // Print bytes and disassembly
        uint8_t instrhex_str[16] = {0};
        instrhex_str[0] = '0';
        instrhex_str[1] = 'x';
        for(uint8_t i = 0; i<instr_len; i++)
        {
            snprintf(instrhex_str+2+2*i,16-2-2*i,"%02x",buffer[pc+i]); // Format hex big-endian
        }

        printf("%16s   %s\n",instrhex_str,asm_buf);
        pc += instr_len;
    }
 ...
 */
int decode_coldfire(const uint8_t *buf, int len, int pc, uint8_t *asm_buf)
{
